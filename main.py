...
def submitContestEntry(name, email, province, essay):
    ...
    driver.save_screenshot(
        "filled " + str(datetime.datetime.now(timezone(timedelta(hours=-5.0)))) + ".png"
    )
    if activatedMode:
        driver.find_element(
    ...
    driver.save_screenshot(
        "submitted "
        + str(datetime.datetime.now(timezone(timedelta(hours=-5.0))))
        + ".png"
    )
...
def scheduledSubmission():
    ...
    nextEssayIndex = (
        len(log["submissions"]) - 1
    )  # works cause will be one behind (0-indexed)
    lastSubmissionDate = log["submissions"][nextEssayIndex - 1]["dateTime"]
    nowTime = datetime.datetime.now(timezone(timedelta(hours=-5.0)))

    # runs submission
    if (lastSubmissionDate.split("/")[0] < nowTime.strftime("%d")) or (
        lastSubmissionDate.split("/")[1] != nowTime.strftime("%m")
    ):
        ...
        newestEntry = {
            "essayIndex": nextEssayIndex,
            "dateTime": nowTime.strftime("%d/%m/%Y"),
            "essaySubmitted": essays[str(nextEssayIndex)],
        }
...
def delayedSubmission():
    ...
    print(
        "Delayed submission activated, waiting "
        + sleepDelay
        + " seconds ("
        + (sleepDelay / 3600)
        + ") hours"
